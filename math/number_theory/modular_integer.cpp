...
public:
    ...
    explicit modular_integer(std::int64_t val) : val(val % get_mod()) {
    ...
    void operator/=(modular_integer other) {
        ...
        *this *= modular_integer(extended_gcd(other.val, get_mod())[1]);

    }

    std::optional<modular_integer> sqrt() const {

        using arr_t = std::array<modular_integer, 2>;
        ...
        if (exponentiate(*this, (get_mod() - 1) / 2, modular_integer(1)).val != 1) {
            return std::nullopt;
        }

        modular_integer a;
        std::int32_t pow = (get_mod() + 1) / 2;
        arr_t res({modular_integer(1), modular_integer()});

        do {
            a = modular_integer(rng());
        } while (
            exponentiate(
                a * a - *this, (get_mod() - 1) / 2, modular_integer(1)
            ).val != get_mod() - 1
        );

        arr_t base({modular_integer(a), modular_integer(1)});
        ...
        return std::optional(modular_integer(std::min(res[0].val, get_mod() - res[0].val)));

    }

    friend bool operator==(modular_integer lhs, modular_integer rhs) {
